...
* 字符值过滤
*/
function filter_var_value($value){
...
function get_image_path($goods_id, $image='', $thumb=false, $call='goods', $del=false)
{
    ...
    $url = (empty($image) || $image=='/') ? '/'.$GLOBALS['_CFG']['no_picture'] : $image;
    return $url;
}
...
//判断有没有第五级区域
function is_have_five_city(){
	global $db,$ecs;

	$sql = "select count(region_id) as num,parent_id,region_type from ".$ecs->table('region')." where region_type=4 group by parent_id";
	$query = $db->query($sql);
	$ret = array();
	while($row = $db->fetchRow($query)){
		$ret[$row['parent_id']] = $row['num'];
	}
	return $ret;
}
