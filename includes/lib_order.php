...
function order_fee($order, $goods, $consignee)
{
    ...
    // 红包和积分最多能支付的金额为商品总额
	$min_bonus_amount = $total['goods_price'] - $bonus_amount;
    $max_amount = $total['goods_price'] == 0 ? $total['goods_price'] : ($min_bonus_amount >= 0 ? $min_bonus_amount : 0);
 ...
 * 获取订单对应的仓库佣金记录id
 * @param int $suppid  店铺id
 * @param int $storeid  仓库id(分销商)
 */
function get_order_store_rebate($suppid,$storeid){
	$spkey = intval($suppid);
	$storeid = intval($storeid);
	if($storeid<=0){
		return 0;
	}
	$sql = "select rebate_id, rebate_paytime_start, rebate_paytime_end from ". $GLOBALS['ecs']->table('store_rebate') ." where supplier_id='$spkey' and store_id='$storeid' and is_pay_ok=0 order by rebate_id desc limit 0,1";
	$row = $GLOBALS['db']->getRow($sql);
	$nowtime = gmtime();
	...
	else
	{
		...
		while($kkk=='yes')
		{
			if(insert_id_store_rebate($spkey,$storeid) === false){
				$kkk = 'no';
				return 0;
			}
			$sql2 = "select rebate_id, rebate_paytime_start, rebate_paytime_end from ". $GLOBALS['ecs']->table('store_rebate') ." where supplier_id='$spkey' and store_id='$storeid' and is_pay_ok=0 order by rebate_id desc limit 0,1";
...
function  insert_id_store_rebate($supplier_id,$store_id)
{
		$sql="select store_rebate_paytime from ". $GLOBALS['ecs']->table('store_main_rebate') ." where store_id='$store_id'";
		$store_rebate_paytime = $GLOBALS['db']->getOne($sql);
		if(!$store_rebate_paytime){
			//如果没有设置仓库佣金比例
			return false;
		}

		$sql = "select rebate_paytime_start, rebate_paytime_end from ". $GLOBALS['ecs']->table('store_rebate') ." where supplier_id= '$supplier_id' and store_id='$store_id' and is_pay_ok=0 order by rebate_id DESC LIMIT 0,1";
		$row = $GLOBALS['db']->getRow($sql);
		...
		$sql="insert into ". $GLOBALS['ecs']->table('store_rebate') ."(rebate_paytime_start, rebate_paytime_end, supplier_id, store_id) value('$rebate_paytime_start', '$rebate_paytime_end', '$supplier_id', '$store_id') ";
		$GLOBALS['db']->query($sql);
}
...
/**
 * 根据仓库的配送方式id判断是否是门店自提配送方式
 * @param int $id  仓库配送方式id
 * @retrun int 门店自提id
 */

function is_pups($id){
	global $db,$ecs;
	$sql = "select s.shipping_code,s.support_pickup from ".$ecs->table('store_shipping_fee')." as ssf left join ".$ecs->table('shipping')." as s on ssf.shipping_id = s.shipping_id where ssf.fee_id=".$id;
	$info = $db->getRow($sql);
	if($info){
 ...
 * @retrun array 自提点信息
*/
function get_pickup_info($cityid){
	global $db,$ecs;
