...
function is_have_stock_ext($goodId,$attrValue=''){

	return is_have_stock($goodId,$attrValue,true);
}
...
function get_storeid_youhuo($province, $city, $district, $xiangcun, $goods_id, $goods_attr, $supplier_id,$order_id=0)
{
	if($order_id>0){
		//获取订单中商品的库房信息
		if (!empty($goods_attr) && $goods_attr !='-1')
		{
			$ret = get_order_goods_store_info($order_id,$goods_id,$supplier_id,$goods_attr);
		}else{
			$ret = get_order_goods_store_info($order_id,$goods_id,$supplier_id);
		}
		if($ret != false && $ret['store_id']>0){
			return $ret;
		}
	}
...
//获取订单中对应商品的信息
function get_order_goods_store_info($order_id,$goods_id,$supplier_id,$goods_attr=''){
	global $db,$ecs;
	if(!empty($goods_attr)){
		$goods_attr_order = str_replace('|',',',$goods_attr);
		$sql_attr = " and goods_attr='".$goods_attr."' ";
		$sql_attr_order = " and goods_attr_id='".$goods_attr_order."' ";
	}else{
		$sql_attr = '';
		$sql_attr_order ='';
	}
	$sql = "select store_id, store_number from ". $ecs->table('store_goods_stock') ." where goods_id=".$goods_id.$sql_attr.
			" and supplier_id=".$supplier_id." and store_id in(select send_store_id from ".$ecs->table('order_goods')." where order_id=".$order_id." and goods_id=".$goods_id.$sql_attr_order.")";
	$ret = $db->getRow($sql);
...
function insert_is_shouqing($arr)
{
	$stock_num = is_have_stock($arr['goodid'],$arr['attr']);
	if ($stock_num==0){
		return '<div class="shop_over"></div>';
	}
