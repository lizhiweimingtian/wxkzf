...
$not_login_arr = array(
	'login', 'act_login', 'register', 'act_register', 'act_edit_password', 'get_password', 'send_pwd_email', 'password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email', 'clear_history', 'qpassword_name', 'get_passwd_question', 'check_answer', 'check_register', 'oath', 'oath_login', 'other_login', 'ch_email', 'ck_email', 'check_username', 'forget_password', 'getverifycode',
...
/*余额额支付密码_更改_START_www.68ecshop.com*/
'act_forget_pass', 're_pass', 'forget_surplus_password', 'act_forget_surplus_password', 'update_surplus_password', 'act_update_surplus_password', 'verify_reset_surplus_email', 'get_verify_code'
);
/* 余额额支付密码_更改_END_www.68ecshop.com */
...
/* 路由 */
/* hzj add begin 2016/08/16 */
function action_team_manage() {
	include_once (ROOT_PATH . 'includes/lib_clips.php');
	$user_id = $_SESSION['user_id'];
	$smarty = $GLOBALS['smarty'];
	$smarty->assign('info', get_user_default($user_id)); // 获取用户中心默认页面所需的数据
	$smarty->display('hzj_team_like.dwt');
}
function action_team_index() {
	$user_id = $_SESSION['user_id'];
	$db = $GLOBALS['db'];
	$ecs = $GLOBALS['ecs'];
	$falg = $_GET['type'];
	//验证用户是否已开通
	$chk_sql = "select * from " . $ecs->table('hzj_user') . " where user_id='" . $user_id . "' and utype= '".$falg."'";
	$chk_row = $db->getRow($chk_sql);
	if (empty($chk_row)) {//未开通者执行开通注册处理
		$data = array();
		...
		$data['utype'] = $falg;
		//从用户表中查询推荐人
		$userinfo_sql = "SELECT * FROM " .$ecs->table('users'). " WHERE user_id = '$user_id'";
		$userinfo = $db->getRow($userinfo_sql);
		//验证是否存在推荐人
		if (!empty($userinfo['parent_id'])) {
			//验证推荐人是否开通
			$chk_sql = "select * from " . $ecs->table('hzj_user') . " where user_id='" . $userinfo['parent_id'] . "' and utype= '".$falg."'";
			$chk_row = $db->getRow($chk_sql);
			if (!empty($chk_row)) {
				$data['creatorUid'] = $userinfo['parent_id'];
				//验证父级的下级名额是否已满
				$childCount_sql = "select count(id) cnt from " . $ecs->table('hzj_user') . " where parentUid='" . $data['creatorUid'] . "' and utype= '".$falg."'";
				$childcount_row = $db->getRow($childCount_sql);
				if (intval($childcount_row['cnt']) >= 3) {
					//名额已满
					$sql = "select * from " . $ecs->table('hzj_user') . " where upath like '%".$chk_row['upath']."/%' and pathlev <= '".(intval($chk_row['pathlev'])+9)."' and pathlev > '".($chk_row['pathlev'])."' and utype='".$falg."' order by pathlev";
					$subUsers =  $db->getAll($sql);
					foreach ($subUsers as $i => $item) {
						//验证下级名额
						$subs_sql = "select * from " . $ecs->table('hzj_user') . " where parentUid='" . $item['user_id'] . "' and utype= '".$falg."'";
						$subresult =  $db->getAll($subs_sql);
						if (empty($subresult)) {
							$data['parentUid'] = $item['user_id'];
							$data['upath'] = $item['upath'].'/'.$user_id;
							$data['pathlev'] = intval($item['pathlev'])+1;
						} else if (count($subresult) < 3) {
							$u = 0;
							foreach ($subresult as $k => $v) {
								if ($subresult['creatorUid'] == $data['creatorUid']) {
									$u = 1;
								}
							}
							if ($u == 0) {
					...
					//名额未满
					$data['parentUid'] = $data['creatorUid'];
					$data['upath'] = $chk_row['upath'].'/'.$user_id;
					$data['pathlev'] = intval($chk_row['pathlev'])+1;
				}
			}
		}
		$insertsql = 'insert into '.$ecs->table('hzj_user').'(';
		$splitfalg = '';
		$values = '(';
		foreach ($data as $key => $val) {
			$insertsql .= $splitfalg.$key;
			$values .= $splitfalg."'".$val."'";
			$splitfalg = ',';
		}
		$exsql = $insertsql.') values '.$values.')';
		$db->query($exsql);
	}
	$smarty = $GLOBALS['smarty'];
	$smarty->display('hzj_team_index.dwt');
}
/* hzj add end 2016/08/16 */
/* 代码增加_start By www.68ecshop.com */
...
function action_order_detail()
{
	...
	{
		$back_order_count = $db->getOne("select count(*) from " . $ecs->table('back_order') . " where order_id='$order[order_id]' and goods_id='$goods_val[goods_id]' and status_back < 6");
...
function action_act_account()
{
	...
	else
	{
		...
		//68ecshop add start 2015-4-20
		$order['order_id']		 = $surplus['rec_id'].'-'.$amount*100;
		//68ecshop add end 2015-4-20
		// 计算支付手续费用
...
function action_transform_points()
{
	...
	$smarty->assign('action', $GLOBALS['action']);
	$smarty->assign('lang', $_LANG);
	$smarty->display('user_transaction.dwt');
}
...
function action_tg_login_act()
{
	...
	$sql = "select tg.*, tt.type_money, tt.use_start_date, tt.use_end_date from " . $ecs->table('takegoods') . " AS tg " . " left join " . $ecs->table('takegoods_type') . " AS tt " . "on tg.type_id = tt.type_id where tg.tg_sn= '$tg_sn' ";
	$tgrow = $db->getRow($sql);
	if(! $tgrow)
	{
	...
	if($tgrow['tg_order_id'])
	{
		show_message('对不起，该提货券已使用', '请换个券号重新登录', 'user.php?act=tg_login');
	}
	
	$_SESSION['takegoods_sn_www_com'] = $tg_sn;
	$_SESSION['takegoods_id_www_com'] = $tgrow['tg_id'];
