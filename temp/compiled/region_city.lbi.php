...
<div class="region_city" style="z-index:13">
  <div class="selectcity">
    <div style="float:left;font-size:12px;color: #999;"> <?php echo $this->_var['address_title']; ?> </div>
...
?>
      <div class=<?php if ($this->_var['pid'] == $this->_var['showlevel']): ?>"licur"<?php else: ?>"li"<?php endif; ?> onclick="javascript:get_regions(<?php echo $this->_var['pid']; ?>, 0)" id="city_li_<?php echo $this->_var['pid']; ?>" style="display:<?php if ($this->_var['names']): ?>block<?php else: ?>none<?php endif; ?>"><?php echo $this->_var['names']; ?><i></i></div>
...
?>
    <div class="childli" id="city_box_<?php echo $this->_var['did']; ?>" style="display:<?php if ($this->_var['did'] == $this->_var['showlevel']): ?>block<?php else: ?>none<?php endif; ?>"> <?php $_from = $this->_var['levelsinfo']['\$levels']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('iid', 'infos');if (count($_from)):
