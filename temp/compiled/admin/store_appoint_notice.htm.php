...
<table cellpadding=0 cellspacing=0 width="90%" border=0>  
    ...
    <tr> 
	  ...
	  <select name='stock_id' id='stock_id' onchange="getSubStore(this,'sub_id')">
		...
		<?php $_from = $this->_var['store_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'big');if (count($_from)):
    foreach ($_from AS $this->_var['k'] => $this->_var['big']):
?>
		<option value="<?php echo $this->_var['big']['store_id']; ?>"><?php echo $this->_var['big']['store_name']; ?></option>
