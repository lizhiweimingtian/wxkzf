...
if ($_REQUEST['act'] == 'get_goodsInfo_bysn')
{
    ...
    if ($goodsinfo['goods_id']>0 && $goodsinfo['supplier_id'] == $_SESSION['supplier_id'])
	{
...
if ($_REQUEST['act'] == 'list')
{
	...
	/* 入库类型 */
	$sql = "select type_id, type_name from ". $ecs->table('store_inout_type') ." where in_out=2 and is_valid=1 and supplier_id in (-1,".$_REQUEST['storetypeid'].") order by type_id asc";
...
if ($_REQUEST['act'] == 'insert')
{
	...
	$sql = "select count(goods_id) as num from ". $ecs->table('goods') ." where goods_id in (".implode(",",$goodIds).") and supplier_id=".$_SESSION['supplier_id'];
	$num = $db->getOne($sql);
