...
function appoint_check_goods($ordersn,$stockid,$subid){
	...
	while($row = $db->fetchRow($query)){
		...
		if($row['supplier_id'] != $_SESSION['supplier_id']){
	...
	foreach($retinfo as $okey=>$oval){//订单分
		foreach($oval as $skey=>$sval){//订单当中的商品所在库房分
			...
			$insert_inout_list = array(
				...
				'store_id'=>$skey,
				'adminer_id'=>$_SESSION['supplier_user_id'],
				'inout_type'=>INOUT_APPOINT_IN,
				'inout_mode'=>2,
				'order_sn'=>$okey,
				'takegoods_man'=>$_SESSION['supplier_name'],
				'today_sn'=>$in_list_info['today_sn'],
				'add_date'=>$in_list_info['add_date'],
				'add_time'=>$in_time,
				'supplier_id'=>$_SESSION[supplier_id],
				'store_type_id'=>$_REQUEST[storetypeid]
			);
...
function order_list()
{
    ...
    if ($result === false)
    {
		...
		if($_REQUEST['ismanagestore']){
			...
			//租的平台方库
			$sql1 = "select store_id from " . $GLOBALS['ecs']->table('store_adminer') ." where supplier_id=".$_REQUEST[storetypeid];
		}
		...
		if(empty($filter['ssid']) && empty($filter['sid']) && $_SESSION['supplier_admin_id']){
		}
		elseif ($storeid_list)
		{
