...
elseif ($act == 'query')
{
    ...
    make_json_result($smarty->fetch('rebate_store_list.htm'), '',
...
elseif ($act == 'view')
{
	...
	//echo $rebate['store_id'];
	//判断是不是仓库的主管
	$smarty->assign('isqueren',haveDoQueRen($rebate['store_id'],$_SESSION['supplier_user_id'],$_REQUEST[storetypeid]));
...
elseif ($act=='finish')
{
   ...
   $rebate = array(
	...
	$loginfo = array(
		'rebateid'=>$rebate_id,
		'addtime'=>$addtime,
		'reason'=>'入驻方佣金'.createSign($rebates['rebate_id'],$rebates['store_id']).'转帐：'.$rebates['payable_price'],
		'supplier_money'=>$rebates['payable_price'],
		'doman'=>'入驻方:'.$_SESSION['supplier_name'],
		'supplier_id'=>0,
...
function rebate_list($act='')
{
    ...
    if ($result === false)
    {
		...
		$where_store = '';
		if(isSupplierAdminUser()){
			$where_store = '';
		}else{
			$store_ids = loginUserStore($_SESSION['supplier_user_id']);
        ...
        $where = 'WHERE sr.supplier_id='.$_SESSION['supplier_id'].$where_store;
        ...
        /* 查询 */
        $sql = "SELECT sr.*,s.store_name, s.store_id,s.supplier_id, smr.rebate ".
                "FROM " . $GLOBALS['ecs']->table("store_rebate") . " AS  sr left join " .$GLOBALS['ecs']->table("store_main") .  " AS s on sr.store_id=s.store_id left join ".$GLOBALS['ecs']->table("store_main_rebate")." as smr on s.store_id = smr.store_id 
                $where
...
//获取当前登陆人有几个仓库的管理权限
function loginUserStore($supplier_user_id){
	global $db,$ecs;

	$sql = "select store_id from ".$ecs->table('store_adminer')." where admin_id=".$supplier_user_id." and supplier_id=".$_SESSION['supplier_id'];
