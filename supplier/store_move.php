...
elseif($act == 'insert'){
	...
	$store_user_out = $_SESSION['supplier_user_id'];
	$add_time = gmtime();
	...
	foreach ($_POST['goods_id']  AS $gkey=>$gval)
	{
			...
			$sql = "insert into ". $ecs->table('store_inout_goods') ." (inout_rec_id, goods_id, goods_sn, inout_mode, attr_value, number_yingshou, number_shishou,supplier_id,store_type_id ) ".
						"values('$inout_rec_id',  '$goods_id', '$goods_sn', '1', '$attr_value', '$number_shishou', '$number_shishou', '$supplier_id','$_REQUEST[storetypeid]')";
			$db->query($sql);
	}
...
elseif($act == 'check_move_in'){
	...
	$store_user_in = $_SESSION['supplier_user_id'];
	$in_time = gmtime();
...
function getAllStock(){
	...
	/* 仓库 */
	$sql="select store_id, store_name from ". $ecs->table('store_main')." where parent_id='0' and supplier_id=".$_REQUEST[storetypeid];
...
function getUserStock($userid){
	...
	$sql_in = "select sm.store_id from ".$ecs->table('store_adminer')." as sa,".$ecs->table('store_main')." as sm where sa.admin_id=".$userid." and sm.store_id = sa.store_id and sm.parent_id>0 and sa.supplier_id=".$_REQUEST[storetypeid];
...
function getStoreId($userid){
	...
	$sql = "select store_id from ".$ecs->table('store_adminer')." where admin_id=".$userid." and supplier_id=".$_REQUEST[storetypeid];
	$ret = $db->query($sql);
	$array = array();
	while($row = $db->fetchRow($ret)){
		$array[] = $row['store_id'];
...
function getUserName($userid){
	global $db,$ecs;
	$sql = "select user_name from ".$ecs->table('supplier_admin_user')." where user_id=".$userid;
