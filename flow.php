...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
    ...
    if(count($cart_goods)>0){
    	foreach($cart_goods as $key => $val){
    		...
    		//if($val['stock_num']<=0 || $val['stock_num']<$val['goods_number']){
    			//show_message('购物车中商品['.$val['goods_name'].']库存不足!', '', '', 'warning');
    		//}
    		//morestock_morecity end
    		$cart_goods_new[$val['supplier_id']]['goodlist'][] = $val;
    		$cart_goods_new[$val['supplier_id']]['shipping_html'] = insert_get_shop_shipping(array('suppid'=>$val['supplier_id']));
    	}
    }
     ...
     * 计算订单的费用
     */
	// file_put_contents('./999.txt',var_export(array('order'=>$order,'cart_goods'=>$cart_goods,'consignee'=>$consignee),true));
...
elseif ($_REQUEST['step'] == 'select_pickinfo')
{
	...
	$result['content']     = $smarty->fetch('library/pickup.lbi');
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	/* 检查购物车中是否存在赠品，赠品的库存是否充足 */     //jx   2015-1-1
	...
	/* 检查购物车中是否存在赠品，赠品的库存是否充足 */     //jx   2015-1-1 end
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	   ...
	   /*morestock_morecity 把配送方式的价钱算在里面*/
	    ...
	    if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
	    {
			...
			$order_money += $order['surplus'];//累计订单中订单总价
	    }
	...
	//判断用余额支付时，用户的余额是否够
	if ($payment['pay_code'] == 'balance' && $order_money > 0)
	{
		if ($order_money > ($user_info['user_money'] + $user_info['credit_line']))
		{
			show_message($_LANG['balance_not_enough']);
		}
	}
    ...
    foreach($order_info as $ok=>$order){
		...
		//获取仓库佣金id
		$order['store_rebate_id'] = get_order_store_rebate($ok,$order['store_id']);
		...
		if($order['order_amount'] <=0 && $payment['pay_code'] == 'balance'){//余额全额支付
			$split_order['suborder_list'][$ok]['order_amount_formated'] = price_format($order['surplus']);
		}else{
