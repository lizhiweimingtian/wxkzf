...
function reg_by_mobile() {
	...
	for (var i = 4; i < frm.elements.length - 4; i++) // 从第五项开始循环检查是否为必填项
	{
		var needinput = document.getElementById(frm.elements[i].name + 'ii') ? document.getElementById(frm.elements[i].name + 'ii') : '';
		if (needinput != '' && frm.elements[i].value.length == 0) {
			msg += '- ' + document.getElementById(frm.elements[i].name).placeholder + msg_blank + '\n';
		}
	}
