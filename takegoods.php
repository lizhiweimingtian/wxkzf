 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: takegoods.php 17217 2011-01-19 06:29:08Z liubo $
*/
...
$tg_id       = $_SESSION['takegoods_id_www_com'];
...
{
	$_SESSION['takegoods_id_www_com']=0;
	$_SESSION['takegoods_sn_www_com']='';
	header("Location:user.php?act=tg_login");
}
...
if ($_REQUEST['act'] == 'list')
{
	...
	$sql = "select * from ".$ecs->table('takegoods')." where tg_id='$tg_id' ";
	$rowtg = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'take')
{
	...
	$takegoods_sn_www_com = $_SESSION['takegoods_sn_www_com'];
	$takegoods_id_www_com = isset($_POST['takegoods_id_www_com'])  ? intval($_POST['takegoods_id_www_com']) : 0;
    if (empty($takegoods_id_www_com))
	{
		show_message('错误操作', '返回上一页', 'takegoods.php', 'error');
	}
	else
	{ 
		$sql = "select goods_ids from ". $ecs->table('takegoods_goods') ." where tg_id='$takegoods_id_www_com' ";
		$goods_ids = $db->getOne($sql);
		if (empty($goods_ids))
		{
			$sql = "select tgtg.goods_ids from ". $ecs->table('takegoods') ." AS tg left join ". $ecs->table('takegoods_type_goods') .
				...
				" where tg.tg_id='$takegoods_id_www_com' ";
			$goods_ids = $db->getOne($sql);
		}
		...
		$sql = "select rec_id from ". $ecs->table('takegoods_order') ." where tg_id='$takegoods_id_www_com' ";
		$take_have = $db->getOne($sql);
		if ($take_have)
		{
			show_message('对不起，该提货卡已经提过货物了，不能重复提货！', '返回上一页', 'takegoods.php', 'error');
		}
	}
    $add_time = gmtime();
	...
	$sql = "insert into ". $ecs->table('takegoods_order') . "(tg_id, tg_sn, user_id, goods_id, goods_name, country, province, city, district, address, consignee,mobile,email,add_time  )".
		" value('$takegoods_id_www_com', '$takegoods_sn_www_com', '$user_id', '$goods_id', '$goods_name', '$country', '$province','$city','$district', '$address', '$consignee' , '$mobile', '$email', '$add_time')";
	$db->query($sql);
	$tg_order_id = $db->insert_id();
	$sql = "update ". $ecs->table('takegoods') ." set tg_order_id='$tg_order_id', used_time='$add_time'  where  tg_id='$takegoods_id_www_com' ";
	$db->query($sql);
	...
	if ($_CFG['takegoods_send_email'] =='1' || $_CFG['takegoods_send_sms'] == '1' )
	{
		...
		{	
			...
			send_mail($_CFG['shop_name'], $admin_email, '卡号：'. $takegoods_sn_www_com .' 提货成功', $content1, 0);
			send_mail($_CFG['shop_name'], $email, '卡号：'. $takegoods_sn_www_com .' 提货成功', $content2, 0);
		}
		if ($_CFG['takegoods_send_sms'] == '1' )
		{
			include_once('includes/cls_sms.php');
			$sms = new sms();
			$sms->send($_CFG['service_phone'], $content2,'', 13,1);
			$sms->send($mobile, $content1,'', 13,1);
		}
	}    
