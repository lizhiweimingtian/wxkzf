<?
	...
	$username = preg_replace("/\s+|^c:\\con|[%,\*\"\s\<\>\&]|$guestexp/is", '', $userinfo['screenName']);//过滤非法字符
	...
	if (register2($username, $password, $email, $userbak, $other) !== false)
	{
		/*把新注册用户的扩展信息插入数据库*/
		...
		show_message(sprintf($_LANG['register_success'], $username . $ucdata), array($_LANG['back_home'], $_LANG['profile_lnk']), array('index.php', 'user.php'), 'info');
	}
	else
	{
		$err->show($_LANG['sign_up'], 'register.php');
	}
