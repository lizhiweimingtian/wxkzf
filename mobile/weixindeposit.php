...
$order = $db->getRow("SELECT * FROM " . $ecs->table('user_account') . " WHERE id = $oid");

//if($order['pay_status'] == 2) exit('is payed');

if ($order['amount'] > 0){

	$order['pay_id'] = $GLOBALS['db']->getOne("select pay_id from ".$GLOBALS['ecs']->table('ecsmart_payment')." where pay_name = '".$order['payment']."'");

	$order['order_amount'] = $order['amount'];

	$order['order_id']	= $order['id'].'-'.$order['amount']*100;

	$order['order_sn']	= $order['id'];

	$payment = ecsmart_payment_info($order['pay_id']);
