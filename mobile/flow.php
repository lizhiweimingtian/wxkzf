...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        $consignee = array(
            ...
            'xiangcun'      => empty($_POST['xiangcun'])   ? '' :   intval($_POST['xiangcun']),
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	//	ecs_header("Location: user.php\n");
        ecs_header("Location: user.php?act=login&jump=balance\n");  //代码修改　返回时跳到结算中心页面
        exit;
    }
...
/* 代码增加_end  By www.68ecshop.com */
    ...
    $consignee = get_consignee($_SESSION['user_id']);
    $province_name = get_region_info($consignee['province']);
    $province_name = empty($province_name)? "" : $province_name."-";
    $city_name = get_region_info($consignee['city']);
    $city_name =  empty($city_name)? "" : $city_name."-";
    $district_name = get_region_info($consignee['district']);
    $district_name =  empty($district_name)? "" : $district_name."-";
    $xiangcun_name = get_region_info($consignee['xiangcun']);
    $xiangcun_name =  empty($xiangcun_name)? "" : $xiangcun_name."-";
    $consignee['region'] = $province_name.$city_name.$district_name.$xiangcun_name;
	/* 代码增加_start  By www.68ecshop.com */
	if (empty($consignee))
	{
		$consignee['country']='1';
        ...
        if(count($cart_goods)>0){
          foreach($cart_goods as $key => $val){
                  ...
                  if($val['stock_num']<=0 || $val['stock_num']<$val['goods_number']){
                    $out_of_goods[$key]['goods_id'] = $val['goods_id'];
                    $out_of_goods[$key]['goods_name'] = $val['goods_name'];
                    $out_of_goods[$key]['goods_number'] = $val['goods_number'];
                    $out_of_goods[$key]['stock_num'] = $val['stock_num'];
                    $out_of_goods[$key]['goods_thumb'] = $val['goods_thumb'];
                    //$smarty->assign('out_of_stock',1);       
                  }
          }
          $smarty->assign('out_of_goods',$out_of_goods);
    }
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
       ...
       //$order['shipping_id'] = intval($_REQUEST['shipping']);
		...
		$result['picktxt']     = '';
		if(is_pups($recid)){
			if(isset($_SESSION['flow_order']['ziti'][$suppid]) && $_SESSION['flow_order']['ziti'][$suppid] > 0){
				$pinfo = get_pickup_one_info($_SESSION['flow_order']['ziti'][$suppid]);
			}else{
				$consignee = get_consignee($_SESSION['user_id']);
				if(isset($consignee['city']) && intval($consignee['city'])>0){
					$pickinfo = get_pickup_info(intval($consignee['city']));
					$pinfo = $pickinfo[0];
				}
			}
			if($pinfo){
				$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pinfo['id']."'><p class='ziti'><span class='zitidian'>自提点：</span><a href='flow.php?step=ziti&pid=".$pinfo['id']."&sid=".$suppid."' class='revise'><span>".$pinfo['shop_name']."</span>修改</a></p>";
			}
		}
...
elseif ($_REQUEST['step'] == 'ziti')
{
	...
	$pickinfo = get_pickup_info($info['city_id']);
...
elseif ($_REQUEST['step'] == 'doziti')
{
	$pid = (isset($_REQUEST['pid'])) ? intval($_REQUEST['pid']) : 0;
    $suppid = (isset($_REQUEST['sid'])) ? intval($_REQUEST['sid']) : 0;
	if($pid > 0){
		$_SESSION['flow_order']['ziti'][$suppid] = $pid;
	}
	ecs_header("Location: flow.php?step=checkout\n");
    exit;
...
function address_user_check($userAddress){
	...
	foreach($_REQUEST['datainfo'] as $k=>$v){
		if(isset($userAddress[$v]) && !empty($userAddress[$v])){
