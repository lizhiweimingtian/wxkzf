...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajax')
{
	...
	$goodslist = category_get_goods($children, $brand, $other_has, $other_youhuo, $price_min, $price_max, $ext, $size, $page, $sort, $order, $limit);
...
/* 页面的缓存ID */
$cache_id = sprintf('%X', crc32($cat_id . '-' . $display . '-' . $sort  .'-' . $order  .'-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' .$other_has. '-'.
...
//LSJ End
    ...
    $goodslist = category_get_goods($children, $brand, $other_has, $other_youhuo, $price_min, $price_max, $ext, $size, $page, $sort, $order);
    if($display == 'grid')
    {
...
//    }
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            "WHERE $where $area_goods $ext ORDER BY $sort $order $limit";
			/* 代码增加_start  By  www.68ecshop.com */
	if ($sort=='salenum')
	{
		$sql = 'SELECT SUM(o.goods_number) as salenum, g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "LEFT JOIN " . $GLOBALS['ecs']->table('order_goods') .  " as o ON o.goods_id = g.goods_id " . "WHERE $where $area_goods $ext group by g.goods_id ORDER BY $sort $order $limit";
	}
