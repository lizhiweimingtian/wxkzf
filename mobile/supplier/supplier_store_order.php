...
require(ROOT_PATH . 'languages/' .$_CFG['lang']. '/supplier_store_order.php');
...
elseif ($_REQUEST['act'] == 'operate2')
{
	...
	if($GLOBALS['db']->query($sql)){
		...
		$rebate_order = array(
			...
			'username' => '入驻方:'.$_SESSION['supplier_name'],
			'type' => REBATE_LOG_ORDER,
			'typedec' => '撤销分销商(仓库)全部佣金',
			'contents' => '相关佣金结算订单全部撤销',
			'addtime' => gmtime()
		);
		$db->autoExecute($ecs->table('store_rebate_log'), $rebate_order, 'INSERT');
...
function getOkOrder(){
    ...
    if ($result === false)
    {
		...
		$sql = "select order_id, order_sn, add_time, order_status, shipping_status, order_amount, money_paid,".
				"pay_status, consignee, address, email, tel, extension_code, extension_id, shipping_time, store_rebate_ispay, " .
...
function getBackOrderMoney(){
	...
	$sql = "select bo.status_refund,(" . order_amount_field('oi.') . ") AS total_fee,oi.pay_id " .
			"from ".$ecs->table('order_info')." as oi,".$ecs->table('back_order')." as bo where oi.store_rebate_id=".$rid." and bo.back_type!=3 and bo.status_back < 5 and oi.order_id=bo.order_id";
	$query = $db->query($sql);
