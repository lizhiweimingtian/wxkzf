...
function get_stock_list($sid){
	/* 库房 */
	global $smarty,$db,$ecs;
	$sql = " select store_id,store_name  from ". $ecs->table('store_main')." where supplier_id=".$_REQUEST[storetypeid]." and ";
	$sql .=  !empty($sid) ? "parent_id='$sid' " : "parent_id>0 ";
	$sub_list=$db->getAll($sql);
	//file_put_contents('../1.txt',var_export($sub_list,true));
	return $sub_list;
}

function assign_store_list($sel_sid)
{
	global $smarty;
	$col_size = 4;
	$store_list = get_store_list();
	array_unshift($store_list,array('store_id'=>'0','store_name'=>'全部仓库'));
	$store_list_new = array();
	$i = 1;
	foreach ($store_list as $key => $val)
	{
		$cur_scr = ceil($i/$col_size);
		$store_list_new[$cur_scr][] = $val;
		if($val['store_id'] == $sel_sid)
		{
			$smarty->assign('sel_store',$val);
		}
		$i ++ ;
	}
	$store_list = $store_list_new;
	$store_list_count = count($store_list);
	$smarty->assign('store_list_count',$store_list_count);
	$smarty->assign('store_list', $store_list);
	$smarty->assign('showck',(!empty($sel_sid) ? 1 : 0));
}
function assign_sub_list($sel_ssid)
{
	global $smarty;
	$col_size = 4;
	$sid = empty($_REQUEST['sid']) ? 0 : intval($_REQUEST['sid']);
	$sub_list = get_stock_list($sid);
	array_unshift($sub_list,array('store_id'=>'0','store_name'=>'全部库房'));
	$sub_list_new = array();
	$i = 1;
	foreach ($sub_list as $key => $val)
	{
		$cur_scr = ceil($i/$col_size);
		$sub_list_new[$cur_scr][] = $val;
		if($val['store_id'] == $sel_ssid)
		{
			$smarty->assign('sel_sub',$val);
		}
		$i ++ ;
	}
	$sub_list = $sub_list_new;
	$sub_list_count = count($sub_list);
	$smarty->assign('sub_list_count',$sub_list_count);
