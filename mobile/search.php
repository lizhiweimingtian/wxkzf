...
else
{
    ...
    $store_chr_ids = $db->getCol("select store_id from ".$ecs->table('store_main')." where parent_id <> 0 and parent_id in (".implode(',',$store_ids).")");
    if(empty($store_chr_ids)){
        $store_chr_ids = array(0);
    }
    
    $stort_goods_ids = $db->getCol("select goods_id from ".$ecs->table('store_goods_stock')." where store_id in  (".implode(',',$store_chr_ids).")");
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        ...
        "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock ." ) ".$tag_where." )".$area_goods;
    $count = $db->getOne($sql);
	...
	if ($sort=='salenum')
	{
		 $sql = "SELECT SUM(o.goods_number) as salenum, g.goods_id, g.goods_name, g.market_price,g.click_count, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
                "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .$area_goods.  " group by g.goods_id " .
            "ORDER BY $sort $order";
	}
