...
function drop_consignee($id)
{
    ...
    else
    {
        ...
        $res = $GLOBALS['db']->query($sql);
        /* 需要清空session中的地址信息 */
        if($_SESSION['flow_consignee']['address_id'] == $id){
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    if (!empty($order['invoice_no']))
    {
         ...
         if (file_exists($plugin))
        {
	      ...
	      $order['invoice_no_old'] = $order['invoice_no'];
...
function get_user_bouns_list($user_id, $num = 10, $start = 0, $suppid=-1)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['supplier_name'] = $row['supplier_id']>0? $GLOBALS['db']->getOne("select supplier_name from ". $GLOBALS['ecs']->table('supplier')." where supplier_id = '".$row['supplier_id']."'"):'网站自营';
        $arr[] = $row;
    }
    return $arr;
