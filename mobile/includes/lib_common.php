...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ,c.type_img ".
    ...
    if ($change_price && defined('ECS_ADMIN') === false)
    {
        switch ($GLOBALS['_CFG']['price_format'])
        {
            ...
            case 1: // 保留不为 0 的尾数
                ...
                break;
            case 2: // 不四舍五入，保留1位
                $price = ($price > 0 ? substr(number_format($price, 2, '.', ''), 0, -1) : 0);
                break;
            case 3: // 直接取整
                $price = intval($price);
                break;
            case 4: // 四舍五入，保留 1 位
                $price = ($price > 0 ? number_format($price, 1, '.', '') : 0);
                break;
            case 5: // 先四舍五入，不保留小数
                $price = round($price);
                break;
...
function get_package_goods($package_id, $package_attr_id='')
{
    ...
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number, p.goods_attr, p.product_number, p.product_id, g.goods_thumb, g.shop_price 
            ...
            WHERE pg.package_id = '$package_id' $sql_package_attr_id ";
    if ($package_id == 0)
    {
...
{
	$mobile_menu = $GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['ecs']->table('ecsmart_menu') . " ORDER BY sort");
	return $mobile_menu;
}
