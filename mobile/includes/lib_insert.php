 ...
 */
 function insert_select_region_category(){
	...
	//获取当前城市下的三级区域
	$sql = "select r.region_id,r.region_name from ".$ecs->table('store_shipping_region')." as ssr 
			left join ".$ecs->table('region')." as r 
			on ssr.district=r.region_id 
			where ssr.district>0 and ssr.city=".$newpcca[2]." 
			group by ssr.district";

	$info1 = array();
	$query = $db->query($sql);
	while($row = $db->fetchRow($query)){
		$info1[$row['region_id']] = $row;
	}

	//获取当前城市下的四级区域
	$info2 = array();
	$sql = "select r.region_id,r.region_name,ssr.district from ".$ecs->table('store_shipping_region')." as ssr 
			left join ".$ecs->table('region')." as r 
			on ssr.xiangcun=r.region_id 
			where ssr.xiangcun>0 and ssr.city=".$newpcca[2]." 
			group by ssr.xiangcun";
	$query = $db->query($sql);
	while($row = $db->fetchRow($query)){
		$info2[$row['district']][$row['region_id']] = $row;
	}
	$smarty->assign('info1',$info1);
	$smarty->assign('info2',$info2);
	$smarty->assign('num',count($info1)+1);

	$val = $smarty->fetch('library/region_city_category.lbi');
...
function insert_select_region($arr){
	...
	$pi = $ci = $di = $xi = '';
	//获取当前城市下的一级区域
	$sql = "select r.region_id,r.region_name,r.region_type from ".$ecs->table('store_shipping_region')." as ssr 
			left join ".$ecs->table('region')." as r 
			on ssr.province=r.region_id 
			where ssr.province>0 group by ssr.province";
	$info1 = array();
	$query = $db->query($sql);
	while($row = $db->fetchRow($query)){
		if($row['region_id'] == $_COOKIE['region_1']){
			$pi = $row['region_name'];
		}
		$info1['province'][$row['region_id']] = $row;
	}

	//获取当前城市下的二级区域
	$sql = "select r.region_id,r.region_name,r.region_type,ssr.province from ".$ecs->table('store_shipping_region')." as ssr 
			left join ".$ecs->table('region')." as r 
			on ssr.city=r.region_id 
			where ssr.city>0 group by ssr.city";
	
	$info2 = array();
	$query = $db->query($sql);
	while($row = $db->fetchRow($query)){
		if($row['region_id'] == $_COOKIE['region_2']){
			$ci = $row['region_name'];
		}
		$info1['province'][$row['province']]['city'][$row['region_id']] = $row;
	}
	
	//获取当前城市下的三级区域
	$sql = "select r.region_id,r.region_name,r.region_type,ssr.city,ssr.province from ".$ecs->table('store_shipping_region')." as ssr 
			left join ".$ecs->table('region')." as r 
			on ssr.district=r.region_id 
			where ssr.district>0 group by ssr.district";
	
	$info3 = array();
	$query = $db->query($sql);
	while($row = $db->fetchRow($query)){
		if($row['region_id'] == $_COOKIE['region_3']){
			$di = $row['region_name'];
		}
		$info1['province'][$row['province']]['city'][$row['city']]['district'][$row['region_id']] = $row;
	}

	//获取当前城市下的四级区域
	$sql = "select r.region_id,r.region_name,r.region_type,ssr.district,ssr.city,ssr.province from ".$ecs->table('store_shipping_region')." as ssr 
			left join ".$ecs->table('region')." as r 
			on ssr.xiangcun=r.region_id 
			where ssr.xiangcun>0 group by ssr.xiangcun";

	$info4 = array();
	$query = $db->query($sql);
	while($row = $db->fetchRow($query)){
		if($row['region_id'] == $_COOKIE['region_4']){
			$xi = $row['region_name'];
		}
		$info1['province'][$row['province']]['city'][$row['city']]['district'][$row['district']]['xiangcun'][$row['region_id']] = $row;
	}

   // file_put_contents('./333.txt',var_export($info1,true));

	$smarty->assign('info',$info1);
	$smarty->assign('showname',$pi.$ci.$di.$xi);
