...
function get_affiliate_ck()
{
	...
	$sql = "select count(*) from (select o.order_id,o.user_id,o.add_time,o.order_status,sum(split_money*goods_number) as total_money,u.user_name,o.is_separate from " . $GLOBALS['ecs']->table('order_info') . " as o ," . $GLOBALS['ecs']->table('order_goods') . " as b," . $GLOBALS['ecs']->table('users') . " as u where o.order_id = b.order_id and o.user_id = u.user_id $sqladd group by o.order_id ) as ab where total_money > 0";
	...
	$sql = "select order_sn,is_separate,order_id,user_id,add_time,order_status,user_name from (select o.order_id,o.order_sn,o.user_id,o.add_time,o.order_status,sum(split_money*goods_number) as total_money,o.is_separate,u.user_name from " . $GLOBALS['ecs']->table('order_info') . " as o ," . $GLOBALS['ecs']->table('order_goods') . " as b," . $GLOBALS['ecs']->table('users') . " as u where o.order_id = b.order_id and o.user_id = u.user_id $sqladd group by o.order_id ) as ab where total_money > 0  ORDER BY order_id DESC" .
