...
function qcode_list()
{
	...
	if ($result === false)
	{
		...
		if($filter['type'] == 0)
		{
			...
			$sql = ("select w.*,IF(w.type=1, g.goods_name, a.title) as title FROM ".$GLOBALS['ecs']->table('weixin_qcode')  ." AS w left join ". $GLOBALS['ecs']->table('goods') ." AS g ON w.content = g.goods_id LEFT JOIN ". $GLOBALS['ecs']->table('ecsmart_article') ." AS a ON w.content = a.article_id") . " LIMIT " . (int)$filter['start'] . ',' . (int)$filter['page_size'];
		}
		$filter['keywords'] = stripslashes($filter['keywords']);
		set_filter($filter, $sql);
	}
