...
function action_insert ()
{
	...
	$sql = "update " . $ecs->table('users') . " set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status' , `mobile_phone`='$mobile_phone' where user_name = '" . $username . "'";
	$db->query($sql);
...
function action_batch_remove ()
{
	...
	{
		$sql = "select user_id from ".$ecs->table('supplier')." where user_id ".db_create_in($_POST['checkboxes']);

		$ret = $db->getCol($sql);

		if(count($ret)>0){
			sys_msg("编号为".implode(',',$ret)."的用户为入驻商，不可以直接删除！");
		}
...
function action_remove ()
{
	...
	$is_supplier = $db->getOne("select count(supplier_id) from ".$ecs->table('supplier')." where user_id=".$_GET['id']);
	if($is_supplier){
		//是入驻商
		sys_msg('此会员是入驻商，请先删除入驻商，才可以删除此会员!');
	}
