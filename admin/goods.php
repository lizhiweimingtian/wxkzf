...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "seller_note,user_rebate_price, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$_POST[user_rebate_price]', '$goods_weight', '$goods_number',".
        ...
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note,user_rebate_price, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_real, " .
    ...
    else
    {
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
				...
				"user_rebate_price = '$_POST[user_rebate_price]', " .
	...
	if($_POST['txm_shu'] && $_POST['tiaoxingm']){//如果txm_shu 和 tiaoxingm存在 就存入  不存在就不执行
		if(isset($_POST['txm_shu']) && isset($_POST['tiaoxingm']) || (empty($_POST['txm_shu'])) && (empty($_POST['tiaoxingm'])) ){
			...
			$db->query("DELETE FROM" .$ecs->table('bar_code')."WHERE goods_id ='$goods_id'");
		}
	}else{
		$db->query("DELETE FROM" .$ecs->table('bar_code')."WHERE goods_id ='$goods_id'");
	}
...
function sendsms_pricecut($goods_id)
{
	...
	while($row=$GLOBALS['db']->fetchRow($res))
	{
		...
		{			
			...
			$parameters = "您关注的商品%s已经降价，您可点击下面链接直接进入商品页面浏览或购买！%s【%s】";

			$pricecut_content = sprintf($parameters,$goods_name,$goods_url,$GLOBALS['_CFG']['shop_name']);
			//$sendstatus = $client->call('Submit',$parameters);

			$ret = sendSMS($row['mobile'],$pricecut_content);
...
function cat_list_to_json($cat_list, $selected = 0)
{
	...
	foreach ($cat_list as $k => $cat)
	{
		...
		$json = sprintf('%s{id: %s, pId: %s, name: "%s", name_pinyin: "%s"},', $json, 

$id, $pId, $name, $name_pinyin);
	}
