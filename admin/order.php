...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('edit_goods' == $step)
    {
        if (isset($_POST['rec_id']))
        {
            foreach ($_POST['rec_id'] AS $key => $rec_id)
            {
			  ...
			  $sql = "select og.goods_id,og.goods_attr_id,og.send_store_number,og.send_store_id,oi.supplier_id ".
			  ...
			  $attr_where = '';
			  if(!empty($orderInfo['goods_attr_id'])){
				  $goods_attr = str_replace(',','|',$orderInfo['goods_attr_id']);
				  $attr_where = ' and goods_attr = "'.$goods_attr.'" ';
			  }
			  $sqlnum = "select store_number ".
				        "from ". $GLOBALS['ecs']->table('store_goods_stock') .
				        " where goods_id=".$orderInfo['goods_id'].$attr_where." and store_id=".$orderInfo['send_store_id']." and supplier_id=".$orderInfo['supplier_id'];
			  /*
              ...
              $goods_number_all = $db->getOne($sqlnum);
              if($goods_number_all>=$goods_number)
              {
				if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
				{
					if($step_act == 'edit'){
						//编辑订单中商品数量时，要时时填加出入库的操作，计算库存
						//对应商品库存变化
						change_order_goods_stock($order_id,$rec_id,$goods_number-$orderInfo['send_store_number'],'edit');
					}
				}
                /* 修改 */
                $sql = "UPDATE " . $ecs->table('order_goods') .
						...
						"send_store_number = '$goods_number' " .
    ...
    elseif ('add_goods' == $step)
    {
       ...
       $res = $db->query($sql);
		...
		if(empty($product_info['goods_attr'])){
			...
			$ext = " AND goods_attr = '".$product_info['goods_attr']."' ";
		}

		$sql = "select stock_id,store_id from ". $ecs->table('store_goods_stock') .
		" WHERE `goods_id` = '" . $goods_id . "' $ext AND store_number=".$product_info['product_number']." AND supplier_id=".$old_order['supplier_id']."  LIMIT 1";
        ...
        /* 插入订单商品 */
            $sql = "INSERT INTO " . $ecs->table('order_goods') .
                        ...
                        "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id,send_store_number,send_store_id) " .
                    "SELECT '$order_id', goods_id, goods_name, goods_sn, " .$product_info['product_id'].", ".
                        "'$goods_number', market_price, '$goods_price', '" .$attr_value . "', " .
                        "is_real, extension_code, 0, 0 , '".implode(',',$goods_attr)."' ,'$goods_number','".$stock_info['store_id']."'" .
        ...
        else
        {
             $sql = "INSERT INTO " . $ecs->table('order_goods') .
                    ...
                    "is_real, extension_code, parent_id, is_gift,send_store_number,send_store_id)" .
                "SELECT '$order_id', goods_id, goods_name, goods_sn, " .
                    "'$goods_number', market_price, '$goods_price', '" . $attr_value. "', " .
                    "is_real, extension_code, 0, 0 ,'$goods_number','".$stock_info['store_id']."'" .
                "FROM " . $ecs->table('goods') .
                " WHERE goods_id = '$goods_id' LIMIT 1";
        }
        $db->query($sql);
        ...
        if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
        {
			...
			if($step_act == 'edit'){
				//编辑订单时，往订单里添加商品
				change_order_goods_stock($order_id,$rec_id,$goods_number,'edit');
			}

			/*$sql = "UPDATE " . $ecs->table('store_goods_stock') .
                    " SET `store_number` = store_number - '" . $goods_number . "' " .
                    " WHERE stock_id=".$stock_info['stock_id'];
            $db->query($sql);*/
        }
    ...
    elseif ('goods' == $step)
    {
        ...
        {
			//将订单中的商品对应的仓库获取到
			if(get_order_stockid($order_id) == false){
				sys_msg('你添加的商品中有没有对应仓库的!', 1);
			}
            //ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=consignee\n");
    ...
    elseif ('consignee' == $step)
    {
        /* 保存订单 */
        $order = $_POST;
		$order['supplier_id'] = $_SESSION['suppliers_id'];//自营商标志
		//获取当前城市所对应的仓库
		$store_id = get_storeid_by_ssq($order['province'], $order['city'], $order['district'],$order['xiangcun'],$order['supplier_id']);
		if($store_id > 0){
			$order['store_id'] = $store_id;
		}else{
			sys_msg('你所选择的地址没有对应的仓库!', 1);
		}
        ...
        {
            /* 下一步 */
            if (1)//exist_real_goods($order_id))
            {
                /* 存在实体商品，去配送方式 */
                //ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=shipping\n");
				ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods\n");
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    elseif ('shipping' == $step)
    {
		...
		$sql = "select ssf.*,s.shipping_name from ".$ecs->table('store_shipping_region')." as ssr,
				".$ecs->table('store_shipping_fee')." as ssf,
...
elseif ($_REQUEST['act'] == 'process')
{
    ...
    if ('drop_order_goods' == $func)
    {
        ...
        if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
        {
             ...
             $db->query($sql);
			 if($step_act == 'edit'){
				 //当编辑订单时，删除订单中商品，商品要入库
				change_order_goods_stock($order_id,$rec_id);
			 }
        }
...
//指库操作
elseif($_REQUEST['act'] == 'appoint'){
	$stock_id = (isset($_POST['stock_id']) && intval($_POST['stock_id'])>0) ? intval($_POST['stock_id']) : 0;
	if($stock_id <=0){
		sys_msg('请选择仓库!');
	}
	$sub_id = (isset($_POST['sub_id']) && intval($_POST['sub_id'])>0) ? intval($_POST['sub_id']) : 0;
	if($sub_id <=0){
		sys_msg('请选择库房!');
	}
	...
	//$ordersn_array = explode(',',$ordersn);
	//验证指定的仓库中的商品是否存在并数量充足
	if(($error = appoint_check_goods($ordersn,$stock_id,$sub_id)) === true){
		$link[0]['text'] = '指定订单操作成功';
		$link[0]['href'] = 'order.php?act=list';
...
* 验证指定订单商品
*/
function appoint_check_goods($ordersn,$stockid,$subid){
	global $db,$ecs,$_CFG;

	$sql = "select og.rec_id,og.order_id,og.goods_id,og.goods_name,og.goods_sn,og.goods_number,og.goods_attr_id,og.send_store_id,oi.order_sn,oi. 	shipping_status,oi.store_id,oi.supplier_id  from ".$ecs->table('order_info').
		   " as oi right join ".$ecs->table('order_goods')." as og on oi.order_id=og.order_id where oi.order_sn in(".$ordersn.")";
	$query = $db->query($sql);
	$error = array();//有问题的订单
	$retinfo = array();
	$orderinfo = array();
	while($row = $db->fetchRow($query)){
		...
		if($row['shipping_status'] != SS_UNSHIPPED){
			//不是未发货状态
			$error['supplier'] = "[".$row['order_sn']."]状态已经不是未发货状态!";
			return $error;
		}

		if($row['supplier_id'] > 0){
			$error['supplier'] = "[".$row['order_sn']."]没有权限操作!";
			return $error;
		}

		$attr_id = $row['goods_attr_id'];
		$attr_where = " and store_id=".$subid;
		if(!empty($attr_id)){
			$attr_id = str_replace(',','|',$attr_id);
			$row['goods_attr_id'] = $attr_id;
			$attr_where .= " and goods_attr='".$attr_id."' ";
		}

		//是否指定的仓库和现在的仓库一样
		if($row['store_id'] == $stockid || $row['send_store_id'] == $subid){
			$error['store'] = "[".$row['order_sn']."]".$row['goods_name']."指定仓库和现在发货仓库相同!";
			return $error;
		}
		//被指的仓库商品数量是否充足
		$stock_sql = "select store_number from ".$ecs->table('store_goods_stock')." where goods_id=".$row['goods_id']." and supplier_id=".$row['supplier_id'].$attr_where;
		$stock_num = $db->getOne($stock_sql);
		if($stock_num < $row['goods_number']){
			$error['num'] = "[".$row['order_sn']."]".$row['goods_name']."指定仓库下库存不足!";
			return $error;
		}else{
			$retinfo[$row['order_sn']][$row['send_store_id']][] = $row;//将操作的订单按照各自的订单商品所在的库房进行分组保存
			$orderinfo[$row['order_id']][] = $row['rec_id'];
		}
	}

	if(empty($error)){
		if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE){//下订单时减库存
			//出库
			if(appoint_order_out($retinfo,$subid)){
				//入库
				if(appoint_order_in($retinfo)){
					//修改订单和商品表中的仓库字段
					if(mode_order_stock($orderinfo,$stockid,$subid) != true){
						$error['inout'] = '订单信息修改失败!';
					}
				}else{
					$error['inout'] = '订单指库入库失败!';
				}
			}else{
				$error['inout'] = '订单指库出库失败!';
			}
		}else{
			//发货减库存,直接修改订单的出库仓库id
			if(mode_order_stock($orderinfo,$stockid,$subid) != true){
...
* 订单指库入库操作
*/
function appoint_order_in($retinfo){
	global $db,$ecs;

	$in_time = gmtime();
	foreach($retinfo as $okey=>$oval){//订单分
		foreach($oval as $skey=>$sval){//订单当中的商品所在库房分
			$in_list_info = createSn('zk');
			$insert_inout_list = array(
				'inout_sn'=>$in_list_info['inout_sn'],
				'inout_status'=>3,//$out_row['inout_status'],
				'store_id'=>$skey,
				'adminer_id'=>$_SESSION['admin_id'],
				'inout_type'=>INOUT_APPOINT_IN,
				'inout_mode'=>2,
				'order_sn'=>$okey,
			...
			foreach($sval as $k=>$v){
				$in_goods = array(
					...
					'goods_sn'=>$v['goods_sn'],
					'goods_id'=>$v['goods_id'],
					'attr_value'=>$v['goods_attr_id'],
					'inout_mode'=>2,
					'number_yingshou'=>$v['goods_number'],
					'number_shishou'=>$v['goods_number'],
					'supplier_id'=>0,
					'store_type_id'=>0
				);
			...
			//入库商品库存变更
			update_stock_in($inout_rec_id, $skey);
...
* 订单指库后修改订单和订单商品表信息
*/
function mode_order_stock($orderinfo,$stockid,$subid){
	global $db,$ecs;
	foreach($orderinfo as $okey=>$oval){
		...
		$db->query('update '.$ecs->table('order_info')." set store_id=".$stockid.",sub_id='".$sub_id."' where order_id=".$okey);//订单修改
		$db->query('update '.$ecs->table('order_goods')." set send_store_id=".$subid." where order_id=".$okey);//订单商品表
	}
	return true;
}
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        //$where = 'WHERE 1 ';
        $where = ($filter['supp']>0) ? 'WHERE o.supplier_id > 0' : 'WHERE o.supplier_id = 0';//morestock_morecity
...
*/
function get_order_stockid($orderId){
	global $db,$ecs;
	$retStr = false;
	$str = '|';
	$sql = "select og.goods_id,og.goods_attr_id,oi.store_id from ".$ecs->table('order_goods')." as og 
			left join ".$ecs->table('order_info')." as oi on og.order_id=oi.order_id where og.order_id=".$orderId." order by og.rec_id";
	$ret = $db->query($sql);
	while($row = $db->fetchRow($ret)){
		if($row['store_id'] > 0){
			$str .= get_order_goods_stockid($row['goods_id'],$row['store_id'],$row['goods_attr_id'])."|";
	...
	if(!empty($str)){
		$str = '|'.$str;
		$sql = "update ".$ecs->table("order_info")." set sub_id='".$str."' where order_id=".$orderId;
		$db->query($sql);
		$retStr = true;
	}
	return $retStr;
...
*@param int $stockId  大仓库id
*@param int $goodsAttrVal  商品属性值
*/
function get_order_goods_stockid($goodsId,$stockId,$goodsAttrVal=''){
	global $db,$ecs;
	if(empty($goodsAttrVal)){
		$and = " ";
	}else{
		$goodsAttrVal = str_replace(',','|',$goodsAttrVal);
		$and = " and sgs.goods_attr='".$goodsAttrVal."' ";
	}
	$sql = "select sgs.store_id from ".$ecs->table('store_goods_stock')." as sgs,
			".$ecs->table('store_main')." as sm 
			where sgs.store_id=sm.store_id 
			and sgs.goods_id=".$goodsId.$and."
			and sm.parent_id=".$stockId." limit 1";
